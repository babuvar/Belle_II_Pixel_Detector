...
for name, fullname in names.items():
    ...
    eps=eff.flatten()
    prf=prof.flatten()
    crm=curr_map.flatten()

    pe=np.multiply(prf,eps)
    sum_ype=np.sum(np.multiply(pe,crm))
    sum_pe_sq=np.sum(np.multiply(pe,pe))
    sum_epp=np.sum(np.multiply(pe,prf))

    ring_score= (sum_epp - sum_ype) / sum_pe_sq
    #ring_score= -1.0 / ring_score
    
    
    fig2=pl.figure(figsize=(10,10))
    pl.imshow(curr_map_copy, interpolation="none",vmin=0, vmax=np.percentile(curr_map_copy, 95)); pl.colorbar(); pl.title('%s \n Ring-score : %s'%(name, ring_score) )
