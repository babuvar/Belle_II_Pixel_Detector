...
This script records pedestals for each value of the current-source range provided.
...
[general]
asicpair = 0                ; [REQUIRED] asicpairs to scan (no delimiters, e.g. 1234, 2, 0 means all.
framenr = 10                ; [REQUIRED] number of frames that will be recorded
triggerfrequency = 80       ; [REQUIRED] trigger frequency in Hz.
useExternalTrigger = False  ; [REQUIRED] Is an external trigger used?
current_source = vnsubout   ; [REQUIRED] What current source is used?

[current_source_ranges]
start = 0                   ; [REQ] starting value
end = 60                    ; [REQ] ending value
step = 4                    ; [REQ] step-size
...
import config_utils
import dhp_utils
from epics_utils import pvlist_get, get_pv
...
def performMeasurement(logger, dhc, dhe, path, asicpairs, framenr, freq, useExternalTrigger, sync,
                       useDHC, useprogresspv, return_dict, event_daq, event_triggering, current_source, current_source_list,
                       current_source_ranges):
    create_context()
    dhes = [module.dhe for module in dhc.modules]


    #Store existing current-source values
    existing_values = epics_utils.pvlist_get(current_source_list)


    with daq.dhh_daq(useDHC=True, wait=0, inputport=dhc.udp_dst_port, dataport=dhc.data_port,
                             controlport=dhc.control_port) as daq_instance:


        #Loop over current-source values
        for current_source_Val in current_source_ranges:
            ...
            #Write new current-source value to DCD via JTAG
            ...
            logger.fine("%s changed to new value: %i" % (current_source, current_source_Val))
            logger.fine("Taking %i memdump frames and saving to %s" % (framenr, fullfilename))
            try:
                triggerTime = 1. / freq
                sleeptime = 1. / freq
                daq.record_memorydump(asicpair=asicpairs, framenr=framenr, dhc=dhc.name, dhePrefix=dhe,
                                         filename=fullfilename, externalTrigger=useExternalTrigger, sync=sync,
                                         trigggerTime=triggerTime, daq_instance=daq_instance, useDHC=useDHC, sleep_time=sleeptime,
                                         readdata=False, useprogresspv=useprogresspv, event_daq=event_daq, event_triggering=event_triggering)
            except IOError:
                return_dict[dhc.name] = {'success': False}

    epics_utils.pvlist_put(existing_values)
    epics_utils.write_to_JTAG_parallel(dhes, 0, JTAGtype="dcd", parallel=True)

    destroy_context()


def main(logger, local_ini, tmp=False, noelog=False, elogcredentials=None, useprogresspv=False, create_pvdump=False,
         dhcs=None):
    if tmp:
    ...
    logger.info("Start of Current-source Scan")
    # ------------ read configuration ----------------------------------------------------------------------------------
    ...
    if not noelog:
        ...
        myelog = None
        try:
            myelog = elog.elog(configfile=local_ini, type="Current Source Calibration Scan", username=credentials.getusername(),
    ...
    if create_pvdump:
        ...
        #Create a dump with a minimal set of PVs
        logger.finest('Creating minimal PV-dump.')
        user = config.get('general', 'user', fallback='PXD')
        #minimal pv-list
        pvlist_min=[]
        try:
            for dhc in dhcs:
                for module in dhc.modules:
                    pvlist_min.append('%s:B:config-%s:device_config:VALUE:set' % (user,module.dhe))
                    pvlist_min.append('%s:B:config-%s:device_module:VALUE:set' % (user,module.dhe))
                    pvlist_min.append('%s:B:config-%s:module_type:VALUE:set' % (user,module.dhe))
            pvlist_min.append('%s:B:config-commitid' %user)
            pvlist_min_vals = pvlist_get(pvlist_min)
            #dictionary to store dump data
            pvdump_dic={}
            for name, val in pvlist_min_vals:
                pvdump_dic[name]=[val, True]
            np.save(os.path.join(path, "PVdump-prescan.npy"), pvdump_dic)
        except KeyError as e:
            logger.warning('Some keys not found, are you sure the DHEs match the ones in the current commit?'
                            'PVDump will not be created!')
...
if __name__ == "__main__":
    ...
    parser.add_argument('-pv', '--createPVdump', action='store_true', dest='createPVdump', default=False,
                        help='Option to create and store a full PV-dump of the system. Otherwise a minimal dump is stored anyway.')
    ...
    main(logger, local_ini, args.temporary, args.noelog, create_pvdump=args.createPVdump)
