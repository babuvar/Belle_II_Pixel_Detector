...
def analysis(logger, module, path, analysispath, current_source_ranges, return_dict, ped_mask_path=None):
    ...
    logger.fine("Calculating Offsets on DHE %s" % module.dhe)
    successful_asicpairs = multiprocessing.Manager().list()
    asic_proceses = []
    for asicpair in asicpairs:
        # analysis_asic(analysispath, asicpair, calibrationData_list, current_source_ranges, expPedestals_list,
        #               ipdac_list, ipdac_ranges, logger, module, nDrains, nGates, offsetMatrixFormat_list, path,
        #               quality_list, successful_asicpairs, user_badPixelMap)
