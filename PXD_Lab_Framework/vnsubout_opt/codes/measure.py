...
def performMeasurement(logger, dhc, dhe, path, asicpairs, framenr, freq, useExternalTrigger, daqport, dataport,
    ...
    for current_source_Val in current_source_ranges:
        ...
        logger.fine("VnSunOut changed to new value: %i" % (current_source_Val))
        ...
        try:
            triggerTime = 1. / freq
            sleeptime = 1. / freq
            data = daq.record_memorydump(asicpair=asicpairs, framenr=framenr, dhc=dhc, dhePrefix=dhe,
...
def main(logger, local_ini, tmp=False, noelog=False, elogcredentials=None, useprogresspv=False, create_pvdump=True,
    ...
    if tmp:
        ...
        if useDHC:
            path = createDataFolder(config['general']['basepath'], None, "pedestal_scan")
        else:
            path = createDataFolder(config['general']['basepath'], config[dhe]['device_module'], "pedestal_scan")
        logger.config("Data will be saved in: %s" % path)
