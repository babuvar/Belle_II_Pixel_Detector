        ...
        for dhc in dhc_objects:
            for module in dhc.modules:
                if "%spath" % module.dhe in return_dict:
                    myelog = elog.elog(local_ini, type="Pedestal Analysis", dhe=module.dhe, dhc_objects=dhc_objects,
...
def analysis(logger, data, acmc_status, offset_status, gateon_voltages, vnsubin, fullpath,
             user, module, config_ana_dict, return_dict, tmp, useprogresspv, pvupdate=False, pdf_plotting=True):
    asicpair = 0
    asicpairs = [1, 2, 3, 4]
    ...
    if config_ana_dict['use_cmc']:
        logger.info("Doing offline common mode correction for %s." % module.dhe)
    ...
    if config_ana_dict['mask_unconnected']:
        ...
        if module.device_type == "pxd9":
            logger.info("Masking unconnected PXD9 channels for %s." % module.dhe)
    ...
    if pvupdate:
        from update import update_ped_monitor
        from epics_utils import get_pv
        logger.info("Update the Pedestal Monitor")
        update_ped_monitor(dhe=module.dhe, pedestals=cell_data_mean, path=path)
        #if get_pv('PXD:M:only-show-ped:S:set').get() is 1:
        #    return 0
    ...
    if pdf_plotting:
        perform_plotting(logger, CM_all, acmc_status, cmc_error, data, module.module_name, module.module_flavor,
                         ...
                         total_number_of_masked_pixels, config_ana_dict, module.dhe)
...
def perform_plotting(logger, CM_all, acmc_status, cmc_error, data, module, module_type, non_cmc_data, offset_status,
                     plot_list, total_mask, gateon_voltages, vnsubin, total_number_of_masked_pixels, c_dict, dhe):
    numberofmaskedchannel = total_number_of_masked_pixels
    asicpair = 0
    if c_dict['plot_frame']:
        logger.info("Creating frame plot for module %s." % dhe)
    ...
    if c_dict['plot_noise']:
        logger.info("Creating noise plot for module %s." % dhe)
        plot_list.append(
    ...
    if c_dict['plot_pedestals']:
        logger.info("Creating pedestal plot for module %s." % dhe)
        plot_list.append(
    ...
    if c_dict['plot_pedestals_mult']:
        logger.info("Creating pedestal plot with single frames (no mean calculated) for module %s." % dhe)
        plot_list.append(
            ("pedestals_mult",
    ...
    if c_dict['plot_everything']:
        logger.info("Creating 'everything' plot for module %s." % dhe)
        plot_list.append(
            (
                "everything",
    ...
    if c_dict['use_cmc'] and not cmc_error:
        try:
            logger.info("Creating Common Mode Comparison plot for module %s." % dhe)
...
def main(logger, local_ini, path=None, tmp=False, noelog=False, dhc_objects=None, elogcredentials=None,
    ...
    # no noise masking for -tmp
    if tmp:
        config_ana_dict['do_noise_masking'] = False
        ...
        for module in dhc.modules:
            ...
            if data_dict[module.dhe] is not None:
                ...
                arguments = [logger, data_dict[module.dhe], acmc_status_dict[module.dhe], offset_status_dict[module.dhe],
                             ...
                             user, module, config_ana_dict, return_dict, tmp, useprogresspv, pvupdate, pdf_plotting]
