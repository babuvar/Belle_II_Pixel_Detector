...
def update_ped_monitor(dhe, pedestals=None, path=None):
    """
    This function allows to update the PV of the Pedestal Monitor

    pedestals have tp be mapped already
    """
    from datetime import datetime
    import mapping
    pedpre = "PXD:M"
    if pedestals is not None:
        get_pv('%s:%s:pedestalmap' % (pedpre, dhe)).put(pedestals.astype(dtype=np.int32).flatten())
        get_pv('%s:%s:last-pedestal-update' % (pedpre, dhe)).put(str(datetime.now()))
        get_pv('%s:%s:last-pedestal-path' % (pedpre, dhe)).put(str(path))

        # Fill the histogram per asic but exclude the six not connected channels (channel 10 to 15!)
        for asic in range(1, 5):
            dcd_frame = mapping.matrixToDcd(pedestals[:, (asic-1)*64:asic*64])
            dcd_frame = np.concatenate([dcd_frame[:, 0:10], dcd_frame[:, 16:]], axis=1)
            histo_data, histo_bins = np.histogram(dcd_frame.flatten(), bins=256, range=(0, 256))
            get_pv('%s:%s:D%i:pedestal:data' % (pedpre, dhe, asic)).put(histo_data)

            # some more info about the pedestal
            number_of_max_code_channels = len(np.where(dcd_frame == 255)[0])
            number_of_min_code_channels = len(np.where(dcd_frame == 0)[0])
            mean = np.mean(dcd_frame)
            stddev = np.std(dcd_frame)

            # put the info in the PVs
            get_pv('%s:%s:D%i:pedestal:maxcodechannel' % (pedpre, dhe, asic)).put(number_of_max_code_channels)
            get_pv('%s:%s:D%i:pedestal:mincodechannel' % (pedpre, dhe, asic)).put(number_of_min_code_channels)
            get_pv('%s:%s:D%i:pedestal:mean' % (pedpre, dhe, asic)).put(mean)
            get_pv('%s:%s:D%i:pedestal:stddev' % (pedpre, dhe, asic)).put(stddev)
...
def main(logger, commitid=-1, file=None, path=None, db=False, system=False, noisemask=False, dhes=None,
         useprogresspv=False, use_udp_upload=False, use_sequential_upload=False, pvupdate=False):
    # if not differently specified, update both: db and system
    if db is False and system is False and pvupdate is False:
        db = True
        system = True
    if pvupdate is True:
        logger.info("Updating Pedestal Monitor.")
    ...
    if system or pvupdate:
        ...
        if pvupdate:
            import mapping
            from naming_utils import dhe_name_to_flavor
            # construct mapper objects
            mapper_dict = {}
            for module_flavour in ['if', 'of', 'ob', 'ib']:
                mapper_dict[module_flavour] = mapping.mapper(module_type='pxd9', module_flavor=module_flavour)
            for dhe in dhes:
                if '%s_D1_pedestal' % dhe in return_dict:
                    pedestals = np.concatenate([return_dict['%s_D%i_pedestal' % (dhe, asic)] for asic in range(1, 5)],
                                               axis=1)
                    mapped_pedestals = mapper_dict[dhe_name_to_flavor(dhe)].raw(pedestals)
                    update_ped_monitor(dhe, mapped_pedestals, path)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--pvupdate', default=False, action='store_true',
                        help="Provides dictionary for pv update of the pedestal Monitor.")
    args = parser.parse_args()

    main(globallogger, args.commitid, args.file, args.path, args.db, args.system, args.noisemask, args.dhe,
         use_udp_upload=args.udp, use_sequential_upload=args.sequential, pvupdate=args.pvupdate)
